...
interface NoticesScrollProps {
  announcements?: NoticeItem[]; 
  role: string | undefined;
  relatedData?: { classes?: { id: string; name: string }[] } | any;
}

const NoticesScroll = ({ announcements = [], role, relatedData }: NoticesScrollProps) => {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [isHovered, setIsHovered] = useState(false);
  const [selected, setSelected] = useState<NoticeItem | null>(null);

  // editing holds the notice data; open controls the boolean setter passed to NoticesForm
  ...
  const displayNotices = [...announcements, ...announcements];
